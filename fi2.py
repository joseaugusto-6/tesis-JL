...
import cv2
import numpy as np
# import subprocess # Ya no es necesario en fi.py si no controla la cámara
# import paho.mqtt.client as mqtt # Ya no es necesario en fi.py si no es cliente MQTT
...
# -- Configuración de la Cámara (para referencia, fi.py no controla la cámara) --
...
# -- Configuración MQTT (para referencia, fi.py no es cliente MQTT) --
...
# CARPETA_LOCAL_ALARMAS # No se usa en fi.py, ya que las imágenes procesadas se suben directamente.
...
try:
    ...
    print(f"[ERROR] Error al cargar modelo YOLOv5: {e}. Asegúrate de tener PyTorch y YOLOv5 configurados.")
    model = None
    class_names = []
...
def procesar_imagenes():
    ...
    while True:
        ...
        for img_dict in imagenes:
            ...
            # Inicializar aquí para el scope del bucle
            known_embeddings = [] 
            known_labels = [] 
