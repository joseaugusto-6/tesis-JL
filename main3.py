    ...
    with camera_status_lock: # Acceder al diccionario global de estados de cámara
        for device_id in devices_from_firestore:
            ...
            devices_with_status.append({
                'id': device_id,
                'mode': mode,
                'is_active': status_info is not None and (datetime.now() - status_info['timestamp']).total_seconds() < 5, # Considerar activa si el último reporte es de hace menos de 60s
